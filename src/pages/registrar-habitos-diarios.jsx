...
const RegistrarHabitosDiarios = ({
  showScreen,
  listaHabitos,
  paginacionHabitos,
  changePage,
  placeHolder,
  searchHabitosByFilter,
  searchHabitosOnCalendar,
}) => {
  ...
  const Item = React.memo(({descripcion, categoria, color, item}) => (
    <TouchableOpacity
      style={styles.item}
      onPress={() => seleccionaHabito(item)}>
      <View style={{width: '20%'}}>
        <View
          style={[
            styles.circle,
            {alignSelf: 'center', backgroundColor: color},
          ]}></View>
      </View>
  ...
  const seleccionaHabito = item => {
    const index = habitosArray.findIndex(
      value => item.ID_HABITO === value.ID_HABITO,
    );
    if (index === -1) {
      setHabitosArray([...habitosArray, item]);
    } else {
      Toast.show({
        type: 'error',
        text1: 'Error',
  ...
  const onClose = dat => {
    const index = habitosArray.findIndex(
      item => item.ID_HABITO === dat.ID_HABITO,
    );
    if (index !== -1) {
      const newArray = [
        ...habitosArray.slice(0, index),
        ...habitosArray.slice(index + 1),
      ];
  ...
  const renderListaHabitos = habitosArray.map((dato, index) => {
    return (
      <View
        style={[styles.selectedItem, {backgroundColor: dato.COLOR}]}
        key={index}>
    ...
    <View
      ...
      <View
        ...
        <TextBoxLabelSelect
          ...
          _fontSize={15}
          _iconSize={30}
          placeHolder={placeHolder}></TextBoxLabelSelect>
        <TextBoxLabel
          value={similarDescripcion}
          onChangeText={setSimilarDescripcion}
          label="Buscar por descripción"
          _height={40}
          _fontSize={15}></TextBoxLabel>
        <View
          style={{
            width: '100%',
            flexDirection: 'row',
          ...
          <PrimaryButton
            ...
            _height={40}
            _width={100}
            onClick={saveHistorialSeguimiento}></PrimaryButton>
          <PrimaryButton
            label="Buscar"
            _height={40}
            _width={100}
            onClick={() =>
              searchHabitosByFilter(similarDescripcion)
            }></PrimaryButton>
          <PrimaryButton
            label="Calendario"
            _height={40}
            _width={100}
            onClick={() =>
              searchHabitosOnCalendar(habitosArray)
            }></PrimaryButton>
        </View>
      </View>
      ...
      <View
        ...
        <Pagination
          paginacionDetalle={paginacionHabitos}
